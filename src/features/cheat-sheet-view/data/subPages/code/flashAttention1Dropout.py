@triton.jit
def flash1_fwd(Q, K, V, O, ell, m, R, alpha: tl.constexpr, N_CTX: tl.constexpr):
# @ref fwd-rng
    R = init_rng_state()
    save_rng_state(R)
# @end
...
# @ref bwd-signature
@triton.jit
def flash1_bwd(Q, K, V, O, dO, ell, m, R, dQ, dK, dV, alpha: tl.constexpr, N_CTX: tl.constexpr):
# @end
# @ref bwd-rng
    restore_rng_state(R)
# @end
...
# @end
            # Use raw scores.
# @ref bwd-prob
            p = tl.exp(s - m_i[:, None]) / ell_i[:, None]
# @end
