...
# @ref flash3-cta-score flash3-consumer-score-cur flash3-consumer-score-next flash3-consumer-wait-score-next flash3-fp8-qk-descale
        # Scores use Dao-style FP8 descales: alpha * d_q * d_k.
...
# @ref flash3-cta-load-kv flash3-cta-commit-kv flash3-cta-wait-v flash3-consumer-wait-vprev flash3-consumer-wait-vlast flash3-fp8-v-load
        # Hopper FP8 follows the tutorial shape: V is described as
        # [HEAD_DIM, N_CTX] and transposed after the descriptor load.
        v = desc_v.load([0, start_n]).T
# @end
# @ref flash3-cta-output flash3-consumer-output-prev flash3-consumer-output-last flash3-fp8-p-cast
        p = p.to(tl.float8_e5m2)
        acc = tl.dot(p, v, acc)
# @end
...
# @ref flash3-fp8-v-desc
    # FP8 mirrors Triton's fused-attention tutorial: V is addressed as
    # [HEAD_DIM, N_CTX] so descriptor loads feed the FP8 dot path.
    desc_v = tl.make_tensor_descriptor(
        V,
        shape=[HEAD_DIM, N_CTX],
        strides=[STRIDE_VD, STRIDE_VM],
...
# @ref flash3-fp8-descale-load
    # The Dao Hopper API supplies descales per batch/effective KV head.
    q_descale = tl.load(Q_DESCALE)
    k_descale = tl.load(K_DESCALE)
    v_descale = tl.load(V_DESCALE)
# @end

# @ref flash3-cta-init-state flash3-consumer-init flash3-fp8-qk-descale
    m_i = tl.full((BLOCK_M,), -float("inf"), tl.float32)
    l_i = tl.zeros((BLOCK_M,), tl.float32)
    acc = tl.zeros((BLOCK_M, HEAD_DIM), tl.float32)
    qk_scale = alpha * q_descale * k_descale
# @end
