...
# @ref step-signature
def muon_step(params, grads, state, lr, momentum=0.95, ns_steps=5, weight_decay=0.0):
    """One Muon step with decoupled weight decay."""
# @end
# @ref no-grad
    with torch.no_grad():
# @end
...
# @end
# @ref adjust-lr
            adjusted_lr = lr * max(1.0, param.size(0) / param.size(1)) ** 0.5
# @end
# @ref update
