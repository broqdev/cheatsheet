...
# @ref step-signature
def sgd_step(params, grads, state, lr, momentum=0.9, dampening=0.0, nesterov=True):
    """One PyTorch-style SGD step with Nesterov momentum."""
# @end
# @ref no-grad
    with torch.no_grad():
# @end
...
# @ref nesterov
            if nesterov:
                b_hat = d_p.add(buffer, alpha=momentum)
            else:
                b_hat = buffer
# @end
# @ref update
            param.add_(b_hat, alpha=-lr)
# @end
# @ref state-store
